...
class PatrolNode(Node):
    ...
    def rotate_to_yaw(self, target_oz, target_ow):
        ...
        deg = math.degrees(target_yaw)
        self.get_logger().info(f"Rotate target: {deg:.1f}° (map frame)")
        ...
        self.get_logger().info(
            f"  map current={math.degrees(self.current_yaw):.1f}° "
            f"target={math.degrees(target_yaw):.1f}° "
            f"turn={math.degrees(map_diff):.1f}°")
        ...
        while time.time() < deadline and self.is_running:
            ...
            if abs(error) < self.yaw_tolerance:
                self.get_logger().info("  Yaw aligned with odom feedback!")
                break
            if error * last_error < 0.0:
                self.get_logger().info("  Yaw target crossed, stopping turn")
                break
        ...
        if final_error is not None and abs(final_error) >= self.yaw_tolerance:
            ...
            return
        self.get_logger().info(
            f"  Yaw done: corrected error "
            f"{math.degrees(abs(final_error or 0.0)):.1f}°")

    # ========== ArUco-based fine alignment ==========
    ...
    def align_to_marker(self, shelf_id):
        ...
        for i in range(self.aruco_max_iters):
            ...
            self.get_logger().info(
                f"  Iter {i}: marker at {math.degrees(angle):.2f}°, rotating")
